...
# Install basics
RUN apt-get update \
    && apt-get install -y git wget curl zip unzip ruby ruby-dev gcc make \
...
# Install Android SDK
RUN cd /opt \
    && wget --output-document=android-sdk.tgz --quiet http://dl.google.com/android/android-sdk_r24.3.3-linux.tgz \
...
# Install sdk elepments
COPY tools /opt/tools

RUN ["/opt/tools/android-accept-licenses.sh", "android update sdk --all --no-ui --filter platform-tools,tools,build-tools-26.0.0,android-26,extra-android-support,extra-android-m2repository,extra-google-m2repository"]
